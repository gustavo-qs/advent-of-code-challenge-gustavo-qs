...
for line in sys.stdin:
  if(line.strip() and aux):
    ...
    if(not line.strip()):
      ...
      if(move_qty == 1):
        for index, crate in enumerate(inverted_grid[col[move_from]][:move_qty]):
          inverted_grid[col[move_to]].insert(0, crate)         
          inverted_grid[col[move_from]][index] = ' '

        inverted_grid = [[c for c in row if c != ' '] for row in inverted_grid]
      else:        
        crates = []
        for i in range(0, move_qty):
          crates.append(inverted_grid[col[move_from]][i])
